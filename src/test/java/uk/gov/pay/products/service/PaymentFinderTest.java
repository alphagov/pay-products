...
public class PaymentFinderTest {
    ...
    public void shouldReturnAList_whenFoundByProductId() throws Exception{
        ...
        List<Payment> paymentList = Arrays.asList(paymentEntity.toPayment());

        when(paymentDao.findByProductId(productId)).thenReturn(paymentList);

        List<Payment> exoectedPaymentList = paymentFinder.findByProductId(productId);

        assertThat(exoectedPaymentList.isEmpty(), is(false));
        assertThat(exoectedPaymentList.get(0).getExternalId(), is(externalId));
        assertThat(exoectedPaymentList.size(), is(1));
    }
}
